...
const UserManagement: React.FC = () => {
  ...
  const cashiers = useMemo(() => users.filter(u => u.role === Role.CASHIER), [users]);
  ...
  return (
    <DashboardLayout title="Manage Cashiers">
      <div className="flex justify-end mb-4">
        <Button onClick={() => openModal()}>Add New Cashier</Button>
      </div>
      ...
      {editingUser && (
        <Modal isOpen={isModalOpen} onClose={closeModal} title={editingUser.id ? 'Edit Cashier' : 'Add New Cashier'}>
          <div className="space-y-4">
            ...
            <Input label="Username" value={editingUser.username} onChange={e => setEditingUser({ ...editingUser, username: e.target.value })} />
             <p className="text-sm text-gray-400">Password would be set via an invite link in a real system.</p>
          </div>
